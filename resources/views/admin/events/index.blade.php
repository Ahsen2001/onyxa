            ...
            <tbody class="divide-y divide-[#F0E6D8]">
                @forelse ($events as $event)
                    <tr>
                        ...
                        <td class="px-5 py-4"><x-ui.status-badge :status="$event->status" /></td>
                        <td class="px-5 py-4">
                            <div class="flex flex-wrap justify-end gap-2">
                                <form method="POST" action="{{ route('admin.events.status', $event) }}" onsubmit="return confirm('Mark this event as completed?');">@csrf @method('PATCH')<input type="hidden" name="status" value="completed"><button class="rounded-lg border border-[#2E7D32]/20 px-3 py-2 font-medium text-[#2E7D32]">Complete</button></form>
                                <form method="POST" action="{{ route('admin.events.status', $event) }}" onsubmit="return confirm('Cancel this event?');">@csrf @method('PATCH')<input type="hidden" name="status" value="cancelled"><button class="rounded-lg border border-red-200 px-3 py-2 font-medium text-red-700">Cancel</button></form>
